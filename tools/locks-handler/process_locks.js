    ...
    try {
        ...
        for (let i = 0; i < files.length; i++){
            ...
            const locks = JSON.parse(json_str.toString()).map((lock) => {
                lock.quantity = BigNumber.from(lock.quantity.hex);
                lock.lock_duration = BigNumber.from(lock.lock_duration.hex);
            ...
            locks.forEach((lock) => {
                ...
                if (totals[sym]){
                    totals[sym] = totals[sym].add(lock.quantity);
        ...
        const weth_balance_safe = BigNumber.from('7350007073554095000000');
        const usdb_balance_safe = BigNumber.from('7760284973700910000000000');
        ...
        const write_log = (sym, total, safe_total) => {
            ...
            console.log(`${sym} : ${ethers.utils.formatEther(total)} accounted for ` +
                `(${ethers.utils.formatEther(safe_total.sub(total))} ${get_word(total, safe_total)})`);
        }
