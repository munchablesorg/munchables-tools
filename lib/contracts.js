...
let _provider;
if (process.env.NODE_URL.substring(0, 4) === 'wss:'){
    console.log(`Using WSS node ${process.env.NODE_URL}`);
    _provider = new ethers.WebSocketProvider(process.env.NODE_URL);
    keepAlive({
        provider: _provider,
        onDisconnect: (err) => {
    ...
    console.log(`Using JSON RPC node ${process.env.NODE_URL}`);
    _provider = new ethers.JsonRpcProvider(process.env.NODE_URL);
}
...
if (!process.env.USDB_CONTRACT){
    console.error(`You must specify USDB_CONTRACT`);
    process.exit(1);
}
if (!process.env.WETH_CONTRACT){
    console.error(`You must specify WETH_CONTRACT`);
    process.exit(1);
}
